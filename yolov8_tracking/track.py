            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        bbox_left = output[0] #x1
                        bbox_top = output[1] #y1
                        bbox_right = output[2] #x2
                        bbox_bottom = output[3] #y2

                        bbox_center_x = round((bbox_left + bbox_right) / 2)
                        bbox_center_y = round((bbox_top + bbox_bottom) / 2)

                        if str(id) in prev_boxes:
                            if (bbox_center_x <= line[0][0]) and (prev_boxes[str(id)]["center_x"] >= line[0][0]):
                                count -= 1
                                prev_boxes[str(id)] = {
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox/seg to image
                            ...
                            annotator.circle((bbox_center_x, bbox_center_y))
